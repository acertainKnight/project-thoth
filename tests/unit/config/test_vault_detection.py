...
class TestVaultDetectionFromEnvironment:
    ...
    def test_obsidian_vault_path_with_tilde(self, tmp_path: Path, monkeypatch):  # noqa: ARG002
...
class TestKnownLocationFallback:
    ...
    def test_known_location_exists(self, monkeypatch, tmp_path: Path):
        ...
        # Check if known location already exists (common in dev environments)
        known_vault = Path.home() / 'Documents' / 'thoth'
        thoth_dir = known_vault / '_thoth'

        # Create if it doesn't exist, otherwise use existing
        if not known_vault.exists():
            known_vault.mkdir(parents=True)
            created_vault = True
        else:
            created_vault = False
        ...
        try:
            ...
            if created_thoth and thoth_dir.exists():
                thoth_dir.rmdir()
            if created_vault and known_vault.exists():
                known_vault.rmdir()
...
class TestVaultDetectionErrorHandling:
    ...
    def test_no_vault_found_error_message(self, monkeypatch, tmp_path: Path):
        ...
        assert 'OBSIDIAN_VAULT_PATH' in error_msg or 'THOTH_VAULT_PATH' in error_msg
...
class TestVaultDetectionLogging:
    ...
    def test_logs_success_from_obsidian_vault_path(
