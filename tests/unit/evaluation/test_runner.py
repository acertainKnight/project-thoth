...
from thoth.analyze.citations.evaluation.runner import (
    run_evaluation,
    main
)
...
class TestRunEvaluation:
    ...
    async def test_run_evaluation_complete_pipeline(self, mock_postgres, monkeypatch):
        ...
        # Patch imports
        monkeypatch.setattr('thoth.analyze.citations.evaluation.runner.Config', lambda: mock_config)
        monkeypatch.setattr(
            'thoth.analyze.citations.evaluation.runner.PostgresService',
            lambda x: mock_postgres
        )
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(
            ...
            lambda x: mock_gt_generator
        )

        # Patch visualization functions to prevent file I/O
        monkeypatch.setattr(
            'thoth.analyze.citations.evaluation.runner.create_evaluation_report',
            Mock()
        )
        monkeypatch.setattr(
            'thoth.analyze.citations.evaluation.runner.plot_calibration_curve',
            Mock()
        )
    ...
    @pytest.mark.asyncio
    async def test_run_evaluation_no_ground_truth_returns_none(self, mock_postgres, monkeypatch):
    ...
    async def test_run_evaluation_creates_output_directory(
        self, mock_postgres, tmp_path, monkeypatch, multiple_ground_truth, multiple_resolution_results
    ):
        """Test evaluation creates output directory with timestamp."""
        mock_config = Mock()
        mock_resolution_chain = AsyncMock()
        mock_resolution_chain.batch_resolve = AsyncMock(return_value=multiple_resolution_results)
        mock_resolution_chain.close = AsyncMock()

        mock_gt_generator = Mock()
        mock_gt_generator.generate_from_database = AsyncMock(return_value=multiple_ground_truth)
...
class TestMainCLI:
    ...
    @patch('thoth.analyze.citations.evaluation.runner.argparse.ArgumentParser.parse_args')
...
class TestAsyncSafety:
    ...
    async def test_concurrent_resolution_safe(
        ...
        mock_resolution_chain = AsyncMock()
        async def mock_batch_resolve(citations, parallel=True):
            # Simulate async work
            import asyncio
            await asyncio.sleep(0.01)
            return [Mock()] * len(citations)
    ...
    @pytest.mark.asyncio
    async def test_database_connection_closed_after_use(self, mock_postgres, monkeypatch):
