...
class TestPDFMonitorBaseline:
    ...
    @pytest.mark.skip(
        reason='Skipping heavy initialization test - will test with new implementation'
    )
...
class TestPDFMonitorNewParameter:
    ...
    def test_pdfmonitor_with_document_pipeline_parameter(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            monitor = PDFMonitor(
                watch_dir=watch_dir, document_pipeline=mock_doc_pipeline
            )
    ...
    def test_pdfmonitor_old_pipeline_parameter_shows_deprecation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with pytest.warns(
                DeprecationWarning,
                match="PDFMonitor parameter 'pipeline' is deprecated",
            ):
                monitor = PDFMonitor(watch_dir=watch_dir, pipeline=mock_pipeline)
    ...
    def test_pdfmonitor_document_pipeline_processes_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mock_doc_pipeline.pdf_tracker.verify_file_unchanged = Mock(
                return_value=False
            )
