    ...
    def test_all_required_services_created(self):
        ...
        for service_name in required_services:
            assert service_name in manager._services, (
                f"Service '{service_name}' not created"
            )
            assert manager._services[service_name] is not None, (
                f"Service '{service_name}' is None"
            )

        # Tag service is optional - may be None if OpenRouter API key not available
        for service_name in optional_services:
            assert service_name in manager._services, (
                f"Optional service '{service_name}' not in _services"
            )
            ...
            assert service is None or hasattr(service, '__class__'), (
                f"Optional service '{service_name}' is invalid"
            )
