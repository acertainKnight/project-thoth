...
class TestOpenRouterMemoryLeak:
    ...
    def test_instance_variables_used(self, mock_openrouter_setup):
        ...
        client = OpenRouterClient(
            api_key='test-key', model='openai/gpt-4', use_rate_limiter=True
        )
    ...
    def test_no_memory_leak_on_deletion(self, mock_openrouter_setup):
        ...
        for i in range(100):
            client = OpenRouterClient(
                api_key=f'test-key-{i}', model='openai/gpt-4', use_rate_limiter=True
            )
    ...
    def test_rate_limiter_disabled(self, mock_openrouter_setup):
        ...
        client = OpenRouterClient(
            api_key='test-key', model='openai/gpt-4', use_rate_limiter=False
        )
    ...
    def test_multiple_clients_independent(self, mock_openrouter_setup):
        ...
        client1 = OpenRouterClient(
            api_key='test-key-1', model='openai/gpt-4', use_rate_limiter=True
        )

        client2 = OpenRouterClient(
            api_key='test-key-2', model='openai/gpt-3.5-turbo', use_rate_limiter=False
        )
        ...
        assert (
            id(client1._rate_limiter) != id(client2._rate_limiter)
            if client2._rate_limiter
            else True
        )
    ...
    def test_generate_method_uses_instance_variables(self, mock_openrouter_setup):
        ...
        with patch.object(client._rate_limiter, 'acquire') as mock_acquire:
            ...
            with patch.object(
                OpenRouterClient.__bases__[1], '_generate'
...
class TestMemoryLeakRegression:
    ...
    def test_no_class_dict_accumulation(self):
        ...
        class_vars = {
            k: v
            for k, v in vars(OpenRouterClient).items()
    ...
    def test_memory_usage_stable_over_time(self):
        ...
        with patch('thoth.utilities.openrouter.httpx.get') as mock_get:
            ...
            for i in range(50):
                client = OpenRouterClient(
                    api_key=f'key-{i}', model='openai/gpt-4', use_rate_limiter=True
                )
