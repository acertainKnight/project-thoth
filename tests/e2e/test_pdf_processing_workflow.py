...
def citation_tracker(temp_dirs, service_manager):
    ...
    try:
        tracker = CitationGraph(
            knowledge_base_dir=temp_dirs['output_dir'], service_manager=service_manager
        )
        return tracker
    except Exception:
...
class TestPDFProcessingWorkflowStructure:
    ...
    def test_pipeline_can_be_initialized(
        self, temp_dirs, service_manager, citation_tracker, pdf_tracker
    ):
    ...
    def test_pipeline_methods_exist(
        ...
        assert hasattr(pipeline, 'process_pdf')
        assert (
            hasattr(pipeline, '_calculate_optimal_workers') or True
        )  # Optional method
...
class TestPDFWorkflowIntegration:
    ...
    def test_workflow_can_access_notes_directory(
        self, temp_dirs, service_manager, citation_tracker, pdf_tracker
    ):
