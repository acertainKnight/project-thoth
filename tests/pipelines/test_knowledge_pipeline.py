...
class TestKnowledgePipeline:
    ...
    def test_ask_knowledge_base(self, knowledge_pipeline):
        with patch.object(knowledge_pipeline.services.rag, 'ask_question') as mock_ask:
            mock_ask.return_value = {
                'question': 'Q',
                'answer': 'A',
                'sources': [{'page_content': 'c', 'metadata': {'title': 'Doc'}}],
            }
    ...
    def test_clear_rag_index(self, knowledge_pipeline):
        with patch.object(knowledge_pipeline.services.rag, 'clear_index') as mock_clear:
            knowledge_pipeline.clear_rag_index()
            mock_clear.assert_called_once()

    def test_get_rag_stats(self, knowledge_pipeline):
        with patch.object(knowledge_pipeline.services.rag, 'get_stats') as mock_stats:
