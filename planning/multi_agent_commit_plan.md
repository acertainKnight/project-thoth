...
5. **`Task` schema fields**: `id: UUID`, `type: Literal[...]`, `payload: dict`, `status: Literal["PENDING","RUNNING","SUCCESS","FAILED"]`, `created_at`, `updated_at`.
6. **`Result` schema** mirrors `Task` plus `output: dict | str` and `metrics: dict[str, float]`.
7. **`BaseAgent` ABC** methods:
   ```python
   class BaseAgent(ABC):
       name: str = "unnamed"
