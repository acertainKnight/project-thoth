...
def backfill_graph_to_postgres():
    ...
    nodes_with_tags = sum(
        ...
        if 'analysis' in d and isinstance(d['analysis'], dict) and d['analysis'].get('tags')
    )
    ...
    try:
        ...
        logger.info("         COUNT(CASE WHEN keywords IS NOT NULL AND jsonb_array_length(keywords) > 0 THEN 1 END) as papers_with_tags")
