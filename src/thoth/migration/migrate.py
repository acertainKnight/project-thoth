...
async def run_migration():
    ...
    try:
        ...
        console.print(f"✓ Database schema ready")

        # 2. Migrate citation graph
        papers_migrated, citations_migrated = await migrate_citation_graph(conn, config)
        console.print(f"✓ Migrated {papers_migrated} papers and {citations_migrated} citations")
        ...
        console.print(f"✓ Migrated {chats_migrated} chat sessions and {messages_migrated} messages")

        # 5. Migrate discovery schedule
        schedule_migrated = await migrate_discovery_schedule(conn, config)
        console.print(f"✓ Migrated discovery schedule")
...
async def create_schema(conn: asyncpg.Connection):
    ...
    # Apply missing tables schema (execute as single block to avoid splitting multi-line statements)
    ...
    if missing_tables_file.exists():
        ...
        try:
            ...
            # If full execution fails, fall back to statement-by-statement with better parsing
            if "already exists" not in str(e).lower():
                console.print(f"[yellow]Warning during schema application: {e}[/yellow]")
    else:
        console.print(f"[yellow]Warning: Missing tables schema not found at {missing_tables_file}[/yellow]")


async def migrate_citation_graph(conn: asyncpg.Connection, config: Config) -> tuple[int, int]:
            ...
            if node.get("markdown_path"):
                ...
                if markdown_file.exists():
                    try:
                        ...
                        console.print(f"[yellow]Could not read markdown {markdown_file}: {e}[/yellow]")
...
async def migrate_processing_status(conn: asyncpg.Connection, config: Config) -> int:
    ...
    tracker_file = config.vault_root / "_thoth" / "data" / "output" / "processed_pdfs.json"
    ...
    for pdf_path, metadata in track(data.items(), description="Migrating processing status..."):
        await conn.execute(
            """
...
async def migrate_chat_history(conn: asyncpg.Connection, config: Config) -> tuple[int, int]:
    ...
    possible_paths = [
        ...
        Path(config.agent_storage_dir) / "chat_history.db" if hasattr(config, 'agent_storage_dir') else None,
    ]
    ...
    if not chat_db:
        console.print(f"[yellow]No chat history database found[/yellow]")
        return 0, 0
...
async def migrate_discovery_schedule(conn: asyncpg.Connection, config: Config) -> int:
    ...
    possible_paths = [
        ...
        Path(config.agent_storage_dir) / "discovery_schedule.json" if hasattr(config, 'agent_storage_dir') else None,
    ]
    ...
    if not schedule_file:
        console.print(f"[yellow]No discovery schedule found[/yellow]")
        return 0
    ...
    schedules = data if isinstance(data, dict) and "source_name" in data else {"default": data}

    for source_name, schedule_info in track(schedules.items(), description="Migrating discovery schedule..."):
        await conn.execute(
            """
            ...
            """,
            source_name if source_name != "default" else schedule_info.get("source", "default"),
...
async def migrate_token_usage(conn: asyncpg.Connection, config: Config) -> int:
    ...
    for user_id, usage_data in track(data.items(), description="Migrating token usage..."):
        await conn.execute(
            """
