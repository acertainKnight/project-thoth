...
async def update_citation_metadata(
    ...
    for citation in citations:
        ...
        try:
            ...
            ''', citing_paper['id'], cited_paper['id'],
                 citation.text,
                 citation.title,
                 authors_json,
                 citation.year,
                 citation.venue or citation.journal,
                 citation.influential_citation_count and citation.influential_citation_count > 0)
...
async def regenerate_note(
    ...
    try:
        ...
        for cit in citations_data:
            import json
            authors = json.loads(cit['extracted_authors']) if cit['extracted_authors'] else None
        ...
        import json
        analysis_dict = json.loads(paper['analysis_data']) if paper['analysis_data'] else {}
...
async def main(limit: int = None, skip_note_regeneration: bool = False):
    """
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("Step 1: Extract citations from markdown files (1 LLM call per paper)")
        logger.info("Step 2: Update citation metadata in database")
        if not skip_note_regeneration:
            logger.info("Step 3: Regenerate all notes with complete data (NO LLM calls)")
        else:
        ...
        for i, paper in enumerate(papers, 1):
            try:
                ...
                if citations:
                    ...
                    logger.info(f"[{i}/{len(papers)}] {paper['title'][:50]}: {updated} citations")
                else:
                    logger.debug(f"[{i}/{len(papers)}] {paper['title'][:50]}: No citations found")
                ...
                try:
                    ...
                    if success:
                        ...
                        if notes_regenerated % 10 == 0:
                            logger.info(f"  Regenerated {notes_regenerated}/{len(papers)} notes...")
                    else:
                        note_errors += 1

                except Exception as e:
                    note_errors += 1
                    logger.error(f"Error regenerating note for {paper['title'][:40]}: {e}")
        ...
        logger.info(f"  • Content analysis: 0 calls (skipped)")
        logger.info(f"  • Note generation: 0 calls (used database data)")
        logger.info("=" * 80)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Reprocess citations without full PDF reprocessing')
    parser.add_argument('--limit', type=int, help='Limit number of papers to process')
    parser.add_argument('--skip-note-regeneration', action='store_true',
    ...
    asyncio.run(main(
        limit=args.limit,
        skip_note_regeneration=args.skip_note_regeneration
    ))
