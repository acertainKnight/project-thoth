...
async def apply_research_question_schema(conn: asyncpg.Connection):
    ...
    schema_file = Path(__file__).parent.parent.parent.parent / "docs" / "architecture" / "research-question-schema.sql"
    ...
    for statement in schema_sql.split(';'):
        ...
        if statement and not statement.startswith('--'):
            try:
                ...
                console.print(f"[green]✓[/green] Executed statement")
            except asyncpg.exceptions.DuplicateTableError:
                console.print(f"[yellow]⊙[/yellow] Table already exists")
            except Exception as e:
                if "already exists" in str(e).lower():
                    console.print(f"[yellow]⊙[/yellow] Object already exists")
                else:
...
async def create_inline_schema(conn: asyncpg.Connection):
    ...
    for idx_name, table_name, column in indexes:
        try:
            await conn.execute(f"CREATE INDEX IF NOT EXISTS {idx_name} ON {table_name}({column})")
    ...
    for table in trigger_tables:
        try:
            ...
            if "already exists" in str(e).lower():
                console.print(f"  [yellow]⊙[/yellow] Trigger for {table} already exists")
            else:
    ...
    sources = [
        {
            ...
            'source_type': 'api',
            'capabilities': {'search_by_keywords': True, 'search_by_authors': True, 'search_by_categories': True},
        ...
        },
        {
            ...
            'source_type': 'api',
            'capabilities': {'search_by_keywords': True, 'search_by_authors': True, 'mesh_terms': True},
        ...
        },
        {
            ...
            'source_type': 'api',
            'capabilities': {'search_by_keywords': True, 'search_by_doi': True, 'citation_data': True},
            'rate_limit_per_minute': 60,
        },
        {
            'name': 'openalex',
            ...
            'source_type': 'api',
            'capabilities': {'search_by_keywords': True, 'search_by_authors': True, 'institution_data': True},
        ...
        },
        {
            ...
            'source_type': 'api',
            'capabilities': {'search_by_keywords': True, 'search_by_authors': True, 'preprints': True},
            'rate_limit_per_minute': 60,
        },
    ]
...
async def run_migration():
    ...
    console.print("\n[bold cyan]═══════════════════════════════════════════════[/bold cyan]")
    console.print("[bold cyan]  Research Question Schema Migration - Week 1  [/bold cyan]")
    ...
    console.print(f"[green]✓[/green] Connecting to database...")
