...
def _get_message_queue_block_id() -> str | None:
    """Get the message_queue block ID from saved configuration."""
    blocks_file = (
        Path(__file__).parent.parent.parent.parent
        / 'scripts'
        / 'shared_memory_blocks.json'
    )
...
def _update_block_content(block_id: str, new_content: str) -> bool:
    ...
    response = httpx.patch(f'{API_BASE}/blocks/{block_id}', json={'value': new_content})
...
def read_messages_for_agent(agent_name: str, status: str | None = None) -> list[dict]:
    """
...
def mark_message_complete(sender: str, receiver: str, timestamp: str) -> bool:
    ...
    if target in content:
        # Replace status line
        new_content = content.replace(f'{target}\nTask:', f'{target}\nTask:')
...
def clear_old_messages(keep_recent: int = 10) -> bool:
    ...
    recent_completed = sorted(completed, key=lambda x: x['timestamp'], reverse=True)[
        :keep_recent
    ]
