    ...
    def input_schema(self) -> dict[str, Any]:
        return {
            ...
            'properties': {
                ...
                'role_filter': {
                    'type': 'string',
                    'enum': ['orchestrator', 'discovery', 'document', 'citation', 'analyst', 'curator', 'maintenance'],
    ...
    async def execute(self, arguments: dict[str, Any]) -> MCPToolCallResult:
        ...
        try:
            ...
            if show_details:
                ...
                lines.append("Use role_filter='role' to see skills for a specific agent role.")
        ...
        try:
            ...
            for skill_id in skill_ids:
                ...
                if skill_content is None:
                    ...
                    if required_tools:
                        results.append(f"   Required tools: {', '.join(required_tools)}")
                    loaded_count += 1
            ...
            if agent_id and all_tools_to_attach:
                ...
                logger.info(f"Attaching {len(unique_tools)} tools to agent {agent_id[:8]}...")
                ...
                if tool_attachment_result["attached"]:
                    results.append(f"🔧 Attached: {', '.join(tool_attachment_result['attached'])}")
                ...
                results.append(f"   To attach tools, call load_skill with your agent_id.")
                results.append(f"   Required tools: {', '.join(set(all_tools_to_attach))}")
            ...
            if tool_attachment_result:
                total_attached = len(tool_attachment_result["attached"]) + len(tool_attachment_result["already_attached"])
    ...
    async def execute(self, arguments: dict[str, Any]) -> MCPToolCallResult:
        ...
        try:
            ...
            if detach_tools and agent_id and all_tools_to_detach:
                ...
                logger.info(f"Detaching {len(unique_tools)} tools from agent {agent_id[:8]}...")
                ...
                if detach_result["not_attached"]:
                    results.append(f"✓ Already removed: {', '.join(detach_result['not_attached'])}")
