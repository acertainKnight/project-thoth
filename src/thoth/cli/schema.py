...
def configure_subparser(subparsers):
    ...
    schema_subparsers = parser.add_subparsers(dest='schema_command', help='Schema command')
    
    # Validate schema command
    validate_parser = schema_subparsers.add_parser(
        'validate',
        help='Validate the schema configuration file'
    )
    ...
    list_parser = schema_subparsers.add_parser(
        'list',
        help='List all available schema presets'
    )
    ...
    set_parser = schema_subparsers.add_parser(
        'set',
        help='Switch to a different preset'
    )
...
def validate_command(args):
    ...
    try:
        ...
        if not schema_path.exists():
            ...
            console.print(f"[yellow]Tip:[/yellow] The schema file will be auto-created on first PDF processing")
            return 1
        ...
        try:
            ...
            console.print(f"[green]✓[/green] Schema file is valid")
            ...
            console.print(f"[red]✗[/red] Invalid JSON in schema file")
            console.print(f"  [dim]Error:[/dim] {e}")
            console.print(f"  [dim]Path:[/dim] {schema_path}")
            return 1
            
        except Exception as e:
            console.print(f"[red]✗[/red] Schema validation failed")
            console.print(f"  [dim]Error:[/dim] {e}")
            console.print(f"  [dim]Path:[/dim] {schema_path}")
            return 1
...
def info_command(args):
    ...
    try:
        ...
        console.print(f"  [cyan]Field List:[/cyan]")
...
def set_command(args):
    ...
    try:
        ...
        if args.preset not in schema_config['presets']:
            ...
            console.print(f"  [yellow]Available presets:[/yellow] {', '.join(available)}")
            return 1
