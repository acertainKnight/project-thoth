...
class LettaModeSelectionScreen(BaseScreen):
    ...
    def compose_content(self) -> ComposeResult:
        ...
        with Vertical(id="self-hosted-section", classes="hidden" if self.deployment_mode == "remote" else ""):
            yield Static(
        ...
        with Vertical(id="remote-section", classes="" if self.deployment_mode == "remote" else "hidden"):
            yield Static(
    ...
    async def _detect_letta(self) -> None:
        ...
        if self.deployment_mode == "local":
            ...
            try:
                ...
                status_widget.update("[dim]Checking for running Letta instance...[/dim]")
            ...
            try:
                ...
                if url:
                    ...
                    available, version, healthy = await LettaDetector.check_server(url, timeout=3)
                    if available and healthy:
                        ...
                        if models:
                            ...
                            logger.info(f"Fetched {len(self.letta_live_models)} live models from remote Letta")
                    else:
                        status_widget.update(
    ...
    def on_radio_set_changed(self, event: RadioSet.Changed) -> None:
        ...
        if selected_id == "mode-cloud":
            ...
            asyncio.create_task(self._focus_widget("#cloud-api-key"))
        elif selected_id == "mode-remote":
            self_hosted_section.add_class("hidden")
            cloud_section.add_class("hidden")
            remote_section.remove_class("hidden")
            asyncio.create_task(self._focus_widget("#remote-url"))
    ...
    async def validate_and_proceed(self) -> dict[str, Any] | None:
        ...
        if self.selected_mode == "remote":
            ...
            if available and healthy:
                ...
                if models:
                    ...
                    logger.info(f"Fetched {len(self.letta_live_models)} models from remote Letta")
            else:
                self.show_warning(
        ...
        try:
            ...
            self.clear_messages()
            self.show_success(
                f"Connected to Letta Cloud (v{version or 'unknown'})"
            )
