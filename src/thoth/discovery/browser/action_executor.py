...
class ActionExecutor:
    ...
    async def _execute_navigate(
        ...
        wait_strategy = action.wait_strategy or WaitStrategy.NETWORKIDLE
    ...
    async def _execute_wait(
        ...
        """Execute wait action."""
        wait_strategy = action.wait_strategy or WaitStrategy.TIMEOUT

        if wait_strategy == WaitStrategy.TIMEOUT:
            # Wait for specified duration
            timeout = action.timeout or 1000
            await asyncio.sleep(timeout / 1000.0)
        elif wait_strategy == WaitStrategy.SELECTOR:
