...
class OpenReviewPlugin(BaseDiscoveryPlugin):
        ...
        try:
            ...
            for note in submissions[: max_results * 2]:  # Get extra to filter
                try:
                    ...
                    if decision_filter and not self._matches_decision(
                        note, decision_filter
                    ):
                        continue

                    # Filter by rating if specified
                    if min_rating and not self._meets_rating_threshold(
                        note, min_rating
                    ):
                        continue
    ...
    def _matches_keywords(self, note, keywords: list[str]) -> bool:
        ...
        for keyword in keywords:
            ...
            if (
                ...
                or keyword_lower in abstract
                or any(keyword_lower in nk for nk in note_keywords_lower)
            ):
                return True
    ...
    def _note_to_metadata(self, note, venue: str) -> ScrapedArticleMetadata | None:
