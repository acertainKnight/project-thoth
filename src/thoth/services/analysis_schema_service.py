...
class AnalysisSchemaService(BaseService):
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            self.load_schema()
            self.logger.info(
                f'AnalysisSchemaService initialized with schema: {self.schema_path}'
            )
    ...
    def _copy_default_template(self) -> None:
        ...
        try:
            ...
            if not template_path.exists():
                self.logger.warning(
                    f'Default schema template not found: {template_path}'
                )
                return
            ...
            self.logger.success(
                f'Copied default analysis schema template to {self.schema_path}'
            )
    ...
    def load_schema(self, force_reload: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.success(
                f'Loaded analysis schema v{self._schema_version} from {self.schema_path}'
            )
    ...
    def get_model_for_preset(self, preset_name: str) -> type[BaseModel]:
        ...
        try:
            ...
            raise ServiceError(
                f'Failed to generate model for preset "{preset_name}": {e}'
            ) from e
    ...
    def _build_pydantic_model(
        ...
        for field_name, field_spec in preset_config['fields'].items():
            ...
            if field_name in base_fields:
                self.logger.debug(
                    f'Skipping base field "{field_name}" - using AnalysisResponse definition'
                )
                continue
        ...
        return create_model(model_name, __base__=AnalysisResponse, **field_definitions)

    def _map_json_type_to_python(
        self, json_type: str, field_spec: dict[str, Any]
    ) -> type:
        """
        ...
        try:
            ...
            if not self._schema_config:
                return [
                    {
                        'name': 'default',
                        'description': 'Default AnalysisResponse schema',
                    }
                ]
