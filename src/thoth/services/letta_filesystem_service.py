...
class LettaFilesystemService(BaseService):
    ...
    def __init__(self, config=None):
        ...
        self._sync_state: dict[
            str, dict[str, Any]
        ] = {}  # file_path -> {size, mtime, letta_file_id}
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            if mode == 'cloud':
                ...
                cloud_api_key = os.getenv('LETTA_CLOUD_API_KEY') or getattr(
                    letta_config, 'cloud_api_key', ''
                )
                ...
                if oauth_enabled and not cloud_api_key:
                    ...
                    self.logger.info(
                        f'Using Letta Cloud with OAuth credentials from {credentials_path}'
                    )
                ...
                server_url = getattr(
                    letta_config, 'server_url', 'http://localhost:8283'
                )
                ...
                self._token = os.getenv('LETTA_API_KEY') or os.getenv(
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.logger.warning(
                        f'Letta health check returned {response.status_code}'
                    )
            except Exception as health_error:
                self.logger.warning(
                    f'Could not verify Letta connection: {health_error}'
                )
    ...
    async def create_folder(
        self, name: str, embedding_model: str = 'text-embedding-3-small'
    ) -> str:
        """
        ...
        try:
            self.logger.info(
                f'Creating Letta folder: {name} with embedding: {embedding_model}'
            )
    ...
    async def get_or_create_folder(
        ...
        try:
            ...
            folders = await asyncio.to_thread(self._letta_client.folders.list)
    ...
    async def upload_file(self, folder_id: str, file_path: Path) -> dict[str, Any]:
        """
        ...
        try:
            ...
            with open(file_path, 'rb') as f:
                result = await asyncio.to_thread(
                    self._letta_client.folders.files.upload, folder_id, file=f
                )
            ...
            if not job_id:
                ...
                self.logger.info(
                    f'File uploaded successfully (no async job): {file_path.name}'
                )
                stat = file_path.stat()
                ...
                while elapsed < max_wait:
                    try:
                        job_status = await asyncio.to_thread(
                            self._letta_client.jobs.retrieve, job_id
                        )

                        if job_status.status == 'completed':
                            self.logger.info(
                                f'File uploaded successfully: {file_path.name}'
                            )
                ...
                try:
                    ...
                    if file_key in self._sync_state:
                        ...
                        if (
                            ...
                            self.logger.debug(
                                f'Skipping unchanged file: {file_path.name}'
                            )
                            stats['skipped'] += 1
                            continue
    ...
    async def attach_folder_to_agent(self, agent_id: str, folder_id: str) -> bool:
        """
        ...
        try:
            ...
            await asyncio.to_thread(
                self._letta_client.agents.folders.attach, agent_id, folder_id
            )
