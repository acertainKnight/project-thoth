...
class RAGGroundTruthPair:
    ...
    ground_truth_doc_titles: List[str]
...
class GroundTruthGenerator:
    ...
    def __init__(self, postgres_service: PostgresService, llm_client: Optional[OpenRouterClient] = None):
        """
    ...
    async def _generate_easy_questions(self, count: int) -> List[RAGGroundTruthPair]:
        ...
        try:
            ...
            for paper in papers:
                ...
                questions_templates = [
                    ...
                    (f"Who are the authors of '{title[:50]}...'?", ', '.join(authors[:3]) if authors else "Unknown"),
                    (f"What year was '{title[:50]}...' published?", str(year) if year else "Unknown"),
                    (f"What is the main topic of the paper '{title[:50]}...'?", abstract[:200] + "..."),
                ]

                # Pick one question template
                question, answer = questions_templates[len(pairs) % len(questions_templates)]
    ...
    async def _generate_medium_questions(self, count: int) -> List[RAGGroundTruthPair]:
        ...
        for topic in topics:
            ...
            try:
                ...
                answer = f"Multiple papers discuss {topic}, including approaches such as " + \
                ...
                logger.error(f"Error generating medium questions for topic {topic}: {e}")
                continue

        return pairs[:count]

    async def _generate_hard_questions(self, count: int) -> List[RAGGroundTruthPair]:
        ...
        try:
            papers = await self.postgres.pool.fetch(query, count * 5)  # Get extra for grouping
            ...
            for i in range(0, len(papers), 5):
                ...
                question = f"How has the research focus evolved in the period {year_range}?"
                answer = f"Research evolution from {min(years)} to {max(years)} shows " + \
    ...
    async def load_ground_truth(self, input_path: Path) -> List[RAGGroundTruthPair]:
