...
async def _save_rag_report(metrics: RAGMetrics, output_dir: Path) -> None:
    ...
    with open(report_path, 'w') as f:
        ...
        f.write(f"Avg Relevant Docs per Query: {metrics.retrieval.avg_relevant_docs_per_query:.2f}\n\n")
        ...
        f.write(f"Mean Average Precision (MAP): {metrics.retrieval.mean_average_precision:.4f}\n\n")
        ...
        f.write(f"Semantic Similarity: {metrics.answer_quality.semantic_similarity_score:.4f}\n")
        f.write(f"Answer Relevance: {metrics.answer_quality.answer_relevance_score:.4f}\n")
        f.write(f"Context Utilization: {metrics.answer_quality.context_utilization_score:.4f}\n")
        f.write(f"Hallucination Rate: {metrics.answer_quality.hallucination_rate:.4f}\n")
        f.write(f"Avg Answer Length (tokens): {metrics.answer_quality.avg_answer_length:.1f}\n\n")

        # Performance Metrics
        f.write("PERFORMANCE METRICS\n")
        f.write("-" * 80 + "\n")
        f.write(f"Avg Retrieval Latency: {metrics.performance.avg_retrieval_latency_ms:.1f} ms\n")
        f.write(f"Avg Generation Latency: {metrics.performance.avg_generation_latency_ms:.1f} ms\n")
        f.write(f"Avg Total Latency: {metrics.performance.avg_total_latency_ms:.1f} ms\n")
        f.write(f"Avg Tokens per Query: {metrics.performance.avg_tokens_per_query:.1f}\n")
        f.write(f"Estimated Avg Cost per Query: ${metrics.performance.avg_cost_per_query:.4f}\n\n")
