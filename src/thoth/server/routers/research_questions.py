...
class ResearchQuestionCreate(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=255, description="Descriptive name for the research question")
    ...
    selected_sources: list[str] = Field(..., min_items=1, description="Source selection: ['*'] for all, or specific sources like ['arxiv', 'pubmed']")
    schedule_frequency: str = Field(default="daily", description="Schedule frequency: daily, weekly, monthly, on-demand")
    schedule_time: Optional[str] = Field(None, pattern=r"^\d{2}:\d{2}$", description="Preferred run time in HH:MM format (24-hour)")
    schedule_days_of_week: Optional[list[str]] = Field(None, description="Days for weekly schedule: ['monday', 'tuesday', ...]")
    min_relevance_score: float = Field(default=0.5, ge=0.0, le=1.0, description="Minimum relevance threshold (0.0-1.0)")
    auto_download_pdfs: bool = Field(default=True, description="Automatically download matching PDFs")
    auto_process_pdfs: bool = Field(default=False, description="Automatically process downloaded PDFs")
    max_articles_per_run: int = Field(default=50, ge=1, le=500, description="Maximum articles per discovery run")
    ...
    def validate_schedule_frequency(cls, v):
        ...
        if v not in valid_frequencies:
            raise ValueError(f"Schedule frequency must be one of: {', '.join(valid_frequencies)}")
        return v
...
class ResearchQuestionUpdate(BaseModel):
    ...
    selected_sources: Optional[list[str]] = Field(None, min_items=1)
    ...
    auto_process_pdfs: Optional[bool] = None
    max_articles_per_run: Optional[int] = Field(None, ge=1, le=500)
...
class ArticleMatchResponse(BaseModel):
    ...
    sentiment_recorded_at: Optional[datetime] = None
    matched_at: datetime
    # Article details
...
async def _get_article_match_repository():
    ...
    from thoth.repositories.article_research_match_repository import ArticleResearchMatchRepository
...
async def create_research_question(
    ...
    try:
        ...
        logger.info(f"Created research question: {question.name} ({question_id}) for user {user_id}")
...
async def list_research_questions(
    ...
    try:
        ...
        logger.debug(f"Retrieved {len(questions)} research questions for user {user_id}")
...
async def update_research_question(
    ...
    try:
        ...
        logger.info(f"Updated research question: {question_id} with fields: {list(update_dict.keys())}")
...
async def get_matched_articles(
    question_id: UUID,
    request: Request,
    min_relevance: Optional[float] = Query(None, ge=0.0, le=1.0, description="Minimum relevance score filter"),
    ...
    try:
        ...
        logger.debug(f"Retrieved {len(matches)} matched articles for question {question_id}")
...
async def get_question_statistics(
    ...
    try:
        ...
        return StatisticsResponse(
            ...
            avg_relevance_score=stats.get('avg_relevance', stats.get('avg_relevance_score')),
