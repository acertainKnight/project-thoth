...
async def run_evaluation(
    ...
    try:
        ...
        logger.info(
            f'   Medium: {sum(1 for c in ground_truth if c.difficulty == "medium")}'
        )
        logger.info(
            f'   Hard: {sum(1 for c in ground_truth if c.difficulty == "hard")}'
        )
        ...
        metrics_by_threshold = calculate_metrics_by_confidence_threshold(
            ground_truth, results, thresholds=[0.3, 0.5, 0.6, 0.7, 0.8, 0.9]
        )
        ...
        logger.info(
            f'API Efficiency: {metrics.api_calls_per_citation:.1f} calls/citation'
        )
        logger.info('=' * 60)
