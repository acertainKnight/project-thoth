    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            'success_rate': (
                ...
                if self.processed_citations > 0 else 0.0
            ),
        }
...
class BatchCitationProcessor:
    ...
    def _mock_resolution(self, citation: Citation) -> ResolutionResult:
        ...
        if success:
            return ResolutionResult(
                ...
                matched_data={
                    'doi': citation.doi or f"10.1234/mock-{hash(citation.title) % 10000}",
    ...
    async def _call_resolver(self, citation: Citation) -> ResolutionResult:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                self.resolver.resolve(citation),
    ...
    async def process_batch(
        ...
        if config.checkpoint_path and config.checkpoint_path.exists():
            ...
            if checkpoint_data:
                ...
                logger.info(f"Resuming from checkpoint: {start_index} citations already processed")
        ...
        for chunk_start in range(start_index, len(citations), config.chunk_size):
            ...
            if (config.checkpoint_path and config.checkpoint_interval > 0 and
                len(results) % config.checkpoint_interval == 0):
    ...
    def load_checkpoint(self, path: Path) -> Optional[List[ResolutionResult]]:
        """
    ...
    def _log_final_stats(self):
        ...
        logger.info(f"Successful: {stats.successful_resolutions} "
