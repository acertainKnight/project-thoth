...
async def run_analysis_evaluation(
    ...
    use_existing_ground_truth: Optional[Path] = None
) -> AnalysisMetrics:
    """
    ...
    try:
        ...
        if use_existing_ground_truth:
            ...
            ground_truth = await gt_generator.load_ground_truth(use_existing_ground_truth)
        else:
...
async def _save_analysis_report(metrics: AnalysisMetrics, output_dir: Path) -> None:
    ...
    with open(report_path, 'w') as f:
        ...
        f.write(f"Required Fields Completeness: {metrics.extraction.required_fields_completeness:.4f}\n")
        f.write(f"Optional Fields Completeness: {metrics.extraction.optional_fields_completeness:.4f}\n")
        f.write(f"Avg Field Confidence: {metrics.extraction.avg_field_confidence:.4f}\n\n")
        ...
        f.write(f"Summary Completeness: {metrics.content_quality.summary_completeness:.4f}\n")
        f.write(f"Key Points Relevance: {metrics.content_quality.key_points_relevance:.4f}\n")
        f.write(f"Key Points Coverage: {metrics.content_quality.key_points_coverage:.4f}\n")
        f.write(f"Methodology Extraction Quality: {metrics.content_quality.methodology_extraction_quality:.4f}\n")
        f.write(f"Tag Appropriateness: {metrics.content_quality.tag_appropriateness:.4f}\n")
        f.write(f"Avg Summary Length (tokens): {metrics.content_quality.avg_summary_length:.1f}\n\n")
        ...
        f.write(f"Strategy Selection Accuracy: {metrics.strategy_efficiency.strategy_selection_accuracy:.4f}\n")
        f.write(f"Direct Strategy Usage: {metrics.strategy_efficiency.direct_strategy_usage:.4f}\n")
        f.write(f"Map-Reduce Strategy Usage: {metrics.strategy_efficiency.map_reduce_strategy_usage:.4f}\n")
        f.write(f"Refine Strategy Usage: {metrics.strategy_efficiency.refine_strategy_usage:.4f}\n\n")

        if metrics.strategy_efficiency.avg_processing_time_by_strategy:
            f.write("Avg Processing Time by Strategy:\n")
            for strategy, time_ms in sorted(metrics.strategy_efficiency.avg_processing_time_by_strategy.items()):
                f.write(f"  {strategy}: {time_ms:.1f} ms\n")
        f.write("\n")

        if metrics.strategy_efficiency.quality_by_strategy:
            f.write("Quality Score by Strategy:\n")
            for strategy, quality in sorted(metrics.strategy_efficiency.quality_by_strategy.items()):
