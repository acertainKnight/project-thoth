...
class PaperDiscoveryRepository(BaseRepository[Dict[str, Any]]):  # noqa: UP006
    ...
    async def record_discovery(
        ...
        try:
            query = """
                INSERT INTO paper_discoveries (paper_id, source_id, discovered_at, source_metadata)
                VALUES ($1, $2, $3, $4)
            ...
            if source_id:
                query = """
                    SELECT EXISTS(
                        ...
                        WHERE paper_id = $1 AND source_id = $2
                    )
                """
                result = await self.postgres.fetchval(query, paper_id, source_id)
            else:
                query = """
                    SELECT EXISTS(
                        SELECT 1 FROM paper_discoveries
                ...
                """
                result = await self.postgres.fetchval(query, paper_id)
        ...
        try:
            if since:
                ...
                query = """
                    ...
                    LIMIT $2 OFFSET $3
                """
                results = await self.postgres.fetch(query, source_id, limit, offset)
    ...
    async def get_recent_discoveries(
        ...
        try:
            ...
            query = """
                ...
                LIMIT $2
            """
            results = await self.postgres.fetch(query, since, limit)
            return [dict(row) for row in results]
    ...
    async def count_discoveries_by_source(
        ...
        try:
            if since:
                ...
                """
                return await self.postgres.fetchval(query, source_id, since) or 0
            else:
                query = """
                    SELECT COUNT(*) FROM paper_discoveries
                    WHERE source_id = $1
    ...
    async def get_discovery_statistics(self, days: int = 30) -> Dict[str, Any]:  # noqa: UP006
        ...
        try:
            ...
            """
            result = await self.postgres.fetchrow(query, since)
        ...
        try:
            if source_id:
                query = """
                    ...
                    LIMIT $2
                """
                results = await self.postgres.fetch(query, source_id, limit)
            else:
                query = """
                    SELECT DISTINCT p.*
                    FROM paper_discoveries pd
