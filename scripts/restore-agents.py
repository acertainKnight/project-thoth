...
def restore_agents(backup_dir, letta_url):
    ...
    for backup_file in sorted(backup_files):
        try:
            ...
            if '/' in agent_name:
                print(f"  ⊘ Skipping: Name contains invalid character '/'")
            ...
            if resp.status_code in [200, 201]:
                ...
                actual_block_count = len(blocks_in_memory) if blocks_in_memory else len(blocks_at_top)
                ...
                if actual_block_count > 0:
                    ...
                    print(f"  ⚠️  WARNING: No memory blocks in response")
                    failed.append(agent_name)
            else:
