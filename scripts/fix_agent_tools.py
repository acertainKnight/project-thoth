...
def update_agent_tools(agent_id, agent_name):
    ...
    multi_agent_tools = [t for t in current_tools if t.get('tool_type') == 'letta_multi_agent_core']
    other_tools = [t for t in current_tools if t.get('tool_type') != 'letta_multi_agent_core']
    ...
    if agent_id == ORCHESTRATOR_ID:
        ...
        print(f"  Action: Keep ONLY send_message_to_agent_and_wait_for_reply")
    else:
        # Specialists: NO multi-agent tools (they don't initiate delegation)
        new_tool_ids = [t['id'] for t in other_tools]
        print(f"  Action: REMOVE all multi-agent tools (specialists don't delegate)")
