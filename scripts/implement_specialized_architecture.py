...
def get_agent_tools(agent_id: str) -> List[str]:
    ...
    return [t['id'] for t in agent_data.get('tools', []) if t.get('tool_type') == 'external_mcp']
    ...
    for tool_id in tool_ids:
        try:
            resp = requests.delete(
                f"{BASE_URL}/v1/agents/{agent_id}/tools/{tool_id}",
                headers=HEADERS
            )
            if resp.status_code not in [200, 204]:
                print(f"    Failed to remove tool {tool_id}: {resp.status_code}")
        except Exception as e:
            print(f"    Error removing tool {tool_id}: {str(e)}")
...
def add_tools_to_agent(agent_id: str, tool_ids: List[str]):
    ...
    for tool_id in tool_ids:
        try:
            ...
            print(f"    Error adding tool {tool_id}: {str(e)}")
...
def implement_architecture():
    ...
    for agent_key, config in TOOL_ASSIGNMENTS.items():
        ...
        if config['agent_id'] is None:
            ...
            if not agent_id:
                print(f"  ✗ Skipping tool assignment (agent creation failed)")
                print()
                continue
        ...
        target_tool_ids = [all_tools[name] for name in target_tool_names if name in all_tools]
        print(f"  Target tools: {len(target_tool_ids)}")

        # Calculate changes
        tools_to_remove = [tid for tid in current_tool_ids if tid not in target_tool_ids]
