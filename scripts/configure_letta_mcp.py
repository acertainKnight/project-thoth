...
THOTH_MCP_URL = (
    'http://thoth-mcp:8000'  # Port 8000 is the HTTP port with streaming support
)
...
def add_mcp_server_to_letta() -> dict[str, Any] | None:
    ...
    print('   Server Name: thoth-research-tools')
    ...
    print(
        f'- LETTA_MCP_SERVERS=[{{"name":"thoth-research-tools","url":"{THOTH_MCP_ENDPOINT}","transport":"sse"}}]'
    )
    print()
    ...
    print('   - Name: thoth-research-tools')
...
def update_docker_compose_env() -> bool:
    ...
    compose_file = (
        '/home/nick-hallmark/Documents/python/project-thoth/docker-compose.letta.yml'
    )
        ...
        for i, line in enumerate(lines):
            if 'letta:' in line:
                ...
                for j in range(i, len(lines)):
                    if (
                        env_marker in lines[j]
                        and 'letta-postgres' not in lines[j - 5 : j]
                    ):
                        ...
                        print()
                        print(
                            '⚠️  You need to restart Letta for changes to take effect:'
                        )
                        print(
...
def main():
    ...
    if response.lower() == 'y':
        if update_docker_compose_env():
            ...
            print('Next steps:')
            print(
                '1. Restart Letta: docker compose -f docker-compose.letta.yml restart letta'
            )
