...
async def backfill_markdown_content(conn: asyncpg.Connection, config: Config) -> int:  # noqa: ARG001
    """
    ...
    for paper in papers:
        try:
            ...
            if '_no_images' not in str(md_path):
                no_images_path = (
                    md_path.parent / f'{md_path.stem}_no_images{md_path.suffix}'
                )
    ...
    logger.success(
        f'✓ Backfilled markdown_content for {updated} papers ({errors} errors)'
    )
    return updated
...
async def generate_embeddings(config: Config, limit: int = None) -> Tuple[int, int]:  # noqa: UP006, RUF013
    """
    ...
    try:
        ...
        for i, paper in enumerate(papers, 1):  # noqa: B007
            try:
                ...
                doc_ids = await services.rag.add_documents_async(
                    documents=documents, paper_id=paper['id']
                )
                ...
                if papers_processed % 5 == 0:
                    logger.info(
                        f'  Processed {papers_processed}/{len(papers)} papers, {total_chunks} chunks...'
                    )
        ...
        logger.success(
            f'✓ Generated embeddings for {papers_processed} papers ({total_chunks} chunks, {errors} errors)'
        )
...
async def main(skip_embeddings: bool = False, limit_embeddings: int = None):  # noqa: RUF013
    """
    ...
    try:
        ...
        if not skip_embeddings:
            papers_processed, chunks_created = await generate_embeddings(
                config, limit_embeddings
            )
        else:
        ...
        if not skip_embeddings:
            logger.info(
                f'✓ embeddings: {papers_processed} papers, {chunks_created} chunks'
            )
        else:
            logger.info(f'⏭  embeddings: skipped')  # noqa: F541
        logger.info('')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Backfill data without full reprocessing'
    )
    parser.add_argument(
        '--skip-embeddings',
        action='store_true',
    ...
    asyncio.run(
        main(
            skip_embeddings=args.skip_embeddings, limit_embeddings=args.limit_embeddings
        )
    )
