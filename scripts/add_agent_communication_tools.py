...
def update_agent_tools(agent_id, agent_name, target_tool_names, all_tools):
    ...
    communication_tools = ['send_message_to_agent_async', 'send_message_to_agent_and_wait_for_reply']
    non_comm_tool_ids = [
        t['id'] for t in current_tools
        if t['name'] not in communication_tools
    ]
    ...
    if resp.status_code in [200, 201]:
        # Verify
        agent = get_agent(agent_id)
        comm_tools = [t['name'] for t in agent['tools'] if t['name'] in communication_tools]
...
def main():
    ...
    if success:
        ...
        print(f"✗ Failed to configure orchestrator")
    ...
    print()
    print("How it works:")
    print("  1. Orchestrator calls: send_message_to_agent_and_wait_for_reply(agent_id=..., message=...)")
